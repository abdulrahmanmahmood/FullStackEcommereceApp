...
const ProductInfoDetails = ({ productDetails }) => {
  ...
  const AddToCardHandler = () => {
    if (!user) {
      // router.replace('/product-details/(.*)','/sign-in')
    ...
    <div>
      {productDetails?.attributes?.title ? (
        <div className=" w-[80%] mx-auto">
          ...
          <button
            className="w-[85%] my-10 mx-auto"
            onClick={() => AddToCardHandler}
          >
            <div className="group flex items-center justify-between gap-4 rounded-lg border border-indigo-600 bg-indigo-600 px-5 py-3 transition-colors hover:bg-transparent focus:outline-none focus:ring" href="">
